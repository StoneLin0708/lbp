...
bool lbpFeatureSvm::train(const char* outFilePath, cv::Ptr<cv::ml::SVM> svm) {
  ...
  // generate labels
  for (i = 0; i < pos.size(); i++) labels.at<float>(i) = label_pos;
  for (i = pos.size(); i < number; i++) labels.at<float>(i) = label_neg;
  ...
  auto dataset = cv::ml::TrainData::create(
      trainData, cv::ml::SampleTypes::COL_SAMPLE, labels);
  svm->train(dataset);
  ...
  cout << " positive sample detected : " << detected << " / " << posData.rows
       << endl;
  success = detected;
  ...
  cout << " negative sample detected : " << detected << " / " << negData.rows
       << endl;
...
bool lbpFeatureSvm::trainP(const char* outFilePath, const char* pcaPath,
...
label_t lbpFeatureSvm::testImage(cv::Mat& image) {
  int i;
  ...
  label_t re = _svm->predict(testMat);
  return re;
}

label_t lbpFeatureSvm::testMap(cv::Rect& area) {
  int i;
  float testData[featureNumber];
...
int lbpFeatureSvm::testMap(lbpFeatureEvaluator& in,
                           std::vector<object>& detected, int delta,
                           double scale) {
  ...
  while (true) {
    while (true) {
      ...
      // t0 = getTickCount();
      for (i = 0; i < featureNumber; i++) testData[i] = in.feature.histogram[i];
      // t1 = getTickCount();
...
int lbpFeatureSvm::testMapO(lbpFeatureEvaluator& in,
                            std::vector<object>& detected, int delta,
                            double scale) {
  in.area = Rect(0, 0, trainW, trainH);
  in.countHistogramArea();
