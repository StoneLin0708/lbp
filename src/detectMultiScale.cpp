...
int detectMultiScale(lbpFeatureSvm& svm, cv::Mat& image, double scale = 1.1,
  ...
  int delta;
  std::cout << "image w = " << image.cols << " h = " << image.rows << '\n';

  while (min_width >= (trainW + 2) * pow(scale, times)) ++times;

  scale_now = pow(scale, times);
  resize(image, testImage,
         cv::Size(image.cols / scale_now, image.rows / scale_now));
  ...
  while (scale_now * trainW < max_width) {
    ...
    // cout << "scale : " << setw(9) <<scale_now << "  find : " <<found.size()
    // << endl;
...
void detectMultiScale(lbpFeatureSvm& svm,cv::Mat& image){
    ...
    while(1){
        ...
        //waitKey(0);
        svm.setMap( evaluator.feature.LBP, evaluator.feature.height,
evaluator.feature.width );
        ...
        for( y=0; y<=yMax; y+=delta){
            for( x=0; x<=xMax; x+=delta){
                ...
                if(respones == label_pos) {
                    found.push_back( Point2d( (int)( (double)x*scale_now),
(int)( (double)y*scale_now) ) );
                }
                //cout << "x = " << x <<endl;
            }
            //cout << "y = " << y <<endl;
        }
        //cout << "scale : " << setw(9) <<scale_now << "  find : "
<<found.size() << endl; scale_now = scale_now*scale;
    }
    ...
    for( i=0; i < number; i++){
        circle(image,Point2d( found[i].x + trainW/2,found[i].y+trainH/2)
,20,Scalar( CV_RGB( 255, 20, 20) ),1,8,0 );
    }
