...
int main(int argc, const char* argv[]) {
  ...
  if (arg1 == s_test) {
    ...
    cout << "  [function] [operation]\n"
         << "   test arga argb argc\n"
         << "   detect arga argb argc\n"
         << "   train arga argb argc\n";
  } else {
    cout << " there is no function called:" << arg1 << '\n'
         << " call help for more infomation\n";
  }
...
int detect(int argc, const char* argv[]) {
  ...
  if (false) {
    for (i = 1; i < 3; i = i + 2) bilateralFilter(test, image, i, i * 2, i / 2);
  }
...
int train(int argc, const char* argv[]) {
  ...
  svm->setTermCriteria(
      cv::TermCriteria(CV_TERMCRIT_EPS, 1000, 1e-7 /*DBL_EPSILON*/));
  ...
  t1 = cv::getTickCount();
  cout << "training end in : " << ((t1 - t0) / cv::getTickFrequency())
       << " sec\n";
  /*if(argc != 4){
      cout << "[positive sample list][negative samples list] [train file
  output]" <<endl; return 1;
  }
