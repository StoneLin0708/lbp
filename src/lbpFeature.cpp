...
lbpFeature::lbpFeature() { _saveCounter = 0; }

void lbpFeature::clear() {
  height = 0;
  width = 0;
...
lbpFeature& lbpFeature::operator=(const lbpFeature& f) {
...
bool lbpFeature::outputFile(const char* path) {
  _outFile.open(path, std::ios::out);
  if (_outFile) {
    return true;
  } else {
    std::cout << "could't open : " << path << '\n';
    return false;
  }
}

int lbpFeature::k(int number) { return histogram[sortList[number]]; }

bool lbpFeature::saveK(void) {
  if (!_outFile) return false;
  _outFile << std::setw(3) << height << std::setw(3) << width;
  int i;
  for (i = 0; i < 20; i++) {
    _outFile << std::setw(4) << sortList[i] << ":" << std::setw(3) << k(i);
  }
  _outFile << std::endl;
  return true;
}
...
lbpFeatureEvaluator::lbpFeatureEvaluator() {}

void lbpFeatureEvaluator::loadImage(const Mat& image, bool toLBP) {
  feature.clear();
  // image.copyTo(_image);
  //_image = image.clone();
...
bool lbpFeatureEvaluator::loadMap(std::vector<unsigned char>& lbp, int rows,
                                  int cols) {
  feature.height = rows;
  feature.width = cols;
  feature.LBP.clear();
...
void lbpFeatureEvaluator::countLBP() {
  int i, j;
  feature.LBP.clear();
...
void lbpFeatureEvaluator::countHistogram() {
  int max, temp, i, j;
  ...
  for (j = 0; j < 256; j++) {
    max = j;
    for (i = j + 1; i < 256; i++) {
      if (feature.histogram[feature.sortList[i]] >
          feature.histogram[feature.sortList[max]])
        max = i;
    }
...
void lbpFeatureEvaluator::countHistogramArea() {
  int i, j, xmax, ymax, width = feature.width;
  memset(feature.histogram, 0, sizeof(feature.histogram));
  // calculate histogram
  xmax = area.x + area.width;
...
void lbpFeatureEvaluator::drawHistogram(const char* title_histogram){
    ...
    for( i=1; i<10; i++){
        line(img,Point(20,300-28*i), Point(570,300-28*i),Scalar(
CV_RGB(240,240,240) ),1,8,0);
    }
    line(img,Point(20,301), Point(570,301),Scalar( CV_RGB(240,240,240) ),1,8,0);
    //histogram line
    for( i=0; i< 256; i++){
        scale = (double)*(histogram +i)/(double)maxValue;
        line(img,Point(30+i*2,300),Point( 30+i*2, 300- (int)(scale*280.0) ),
Scalar( CV_RGB( 255*scale , 10, 255*(1.0-scale) ) ) ,2,8,0);
    }

    imshow(title_histogram,img);
}

void lbpFeatureEvaluator::drawMapValue(const char* title,unsigned char
lbpValue){ Mat img( feature.height, feature.width ,CV_8UC1 ,Scalar(255)); int
i,j; printf("%s : lbpValue = %d\n",title,lbpValue); for(j=0; j<feature.height;
j++){ for(i=0;i<feature.width;i++){ if( feature.lbpMat.at<uchar>(j,i) ==
lbpValue) { img.at<uchar>(j,i) = 0;
                }
            }
        }
...
void lbpFeatureEvaluator::drawMapK(const char* title,unsigned char
K_max,unsigned char K_min){ Mat img( feature.height, feature.width ,CV_8UC1
,Scalar(255));
        ...
        for(j=0; j<feature.height; j++){
            for(i=0;i<feature.width;i++){
                ...
                histogramValue = feature.histogram[
feature.lbpMat.at<uchar>(j,i) ];
...
*/
void lbpFeatureEvaluator::pcaCalculate() {
  Mat pcaSet;
  pcaSet = _image.clone();
  Mat imageOut;
  ...
  cv::PCA pca(pcaSet, Mat(), CV_PCA_DATA_AS_COL);

  cv::normalize(pca.eigenvectors, imageOut, 0, 255, cv::NORM_MINMAX, CV_8UC1);
